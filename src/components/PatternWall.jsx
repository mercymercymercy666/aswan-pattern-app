...
/* ── Tatreez variable-width line layer ──────────────────── */
// Every column always has a line. Grayscale value → line width:
// dark pixel = bold/wide, light pixel = very thin.
// Consecutive cells with the same bucketed width merge into one stroke.
// Uses rawGrid (grayscale 0=dark 1=light) when available, falls back to binary grid.
function TatreezLineLayer({ rawGrid, grid, treeX, treeW, bandH, freq, lineWidth, pixelSize = 1, yOffset = 0, thickness = 1 }) {
  const src = rawGrid ?? (grid ? grid.map(r => r.map(v => v ? 0 : 1)) : null);
  ...
  const cols    = Math.max(0, Math.floor((treeX + treeW - firstX) / pitch));
  const rows    = Math.floor(bandH / pitch);
  const gRows   = src.length;
  const gCols   = src[0]?.length ?? 0;
  if (!gRows || !gCols || cols <= 0 || rows <= 0) return null;

  const patCols = Math.max(1, Math.ceil(cols / block));
  const patRows = Math.max(1, Math.ceil(rows / block));
  const minW    = lineWidth * 0.18;
  const maxW    = Math.max(lineWidth * 2.8, pitch * 0.55) * thickness;
  const LEVELS  = 8;

  const buckets = {};
  const addSeg  = (sw, x, y1, y2) => {
    const key = sw.toFixed(2);
    buckets[key] = (buckets[key] ?? '') + `M${x.toFixed(1)},${y1.toFixed(1)}L${x.toFixed(1)},${y2.toFixed(1)}`;
  };
  ...
  for (let ci = 0; ci < cols; ci++) {
    ...
    for (let ri = 0; ri <= rows; ri++) {
      ...
      if (ri < rows) {
        const pr    = Math.floor(ri / block);
        const gr    = Math.min(gRows - 1, Math.floor((pr + 0.5) / patRows * gRows));
        const dark  = 1 - src[gr][gc];              // 0=light → 1=dark
        const level = Math.floor(dark * LEVELS) / LEVELS;
        swStr = (minW + (maxW - minW) * level).toFixed(2);
      }
      if (swStr !== runSw) {
        if (runStart !== -1 && runSw !== null)
          addSeg(Number(runSw), sx, yOffset + runStart * pitch, yOffset + ri * pitch);
        runStart = ri;
        runSw    = swStr;
      }
    }
